  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const meRes = await fetch(`/api/tenant/me${emailParam}`);
        if (!meRes.ok) {
          setIsLoading(false);
          return;
        }
        ...
        const ctx: TenantContext = {
          ...
          profileId: d.profileId || user?.id || null,
        ...
        if (!ctx.workspaceId || !ctx.id || ctx.id === "demo-tenant-id") {
          setRooms([]);
          return;
        }
        ...
        if (ctx.profileId) params.set("profileId", ctx.profileId);

        const res = await fetch(`/api/chat/rooms?${params.toString()}`);
        if (res.ok) {
          const result = await res.json();
          if (result.data && Array.isArray(result.data) && result.data.length > 0) {
            const loadedRooms = mapRooms(result.data, ctx);
            setRooms(loadedRooms);
            setSelectedRoomId(loadedRooms[0].id);
          }
        }
      } catch (err) {
        console.error("Error loading resident chat rooms:", err);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <aside className="w-72 shrink-0 border-r border-slate-200 bg-[#F8FAFC] flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-5">
          <div>
            ...
            {isLoading ? (
              <div className="px-2 py-3 text-[11px] text-slate-400 flex items-center gap-2">
