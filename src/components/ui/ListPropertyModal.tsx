...
export default function ListPropertyModal({
  ...
  const [pricingTiers, setPricingTiers] = useState<Array<{ name: string; rent: string; deposit?: string }>>([]);

  const addPricingTier = (name = "", rent = "", deposit = "") => {
    setPricingTiers((prev) => [...prev, { name, rent, deposit }]);
  };

  const updatePricingTier = (index: number, field: "name" | "rent" | "deposit", value: string) => {
  ...
  useEffect(() => {
    if (isOpen && editingListing) {
      ...
      if (Array.isArray(editingListing.pricingTiers) && editingListing.pricingTiers.length > 0) {
        setPricingTiers(
          editingListing.pricingTiers.map((pt: any) => ({
            name: pt.name || "",
            rent: pt.rent !== undefined && pt.rent !== null ? String(pt.rent) : "",
            deposit: pt.deposit !== undefined && pt.deposit !== null ? String(pt.deposit) : "",
          }))
        );
      } else {
        setPricingTiers([]);
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    if (currentStep === 1) {
      ...
      const validTiers = pricingTiers.filter((pt) => pt.name.trim() && pt.rent);
      if (!rent && validTiers.length > 0) {
        const minRent = Math.min(...validTiers.map((t) => parseFloat(t.rent) || 0));
        setRent(String(minRent));
      }
  ...
  const handlePublish = async () => {
    ...
    const validTiers = pricingTiers
      .filter((pt) => pt.name.trim() && pt.rent)
      .map((pt) => ({
        name: pt.name.trim(),
        rent: parseFloat(pt.rent) || 0,
        deposit: pt.deposit ? parseFloat(pt.deposit) : null,
      }));

    const finalRent = rent
      ? parseFloat(rent)
      : validTiers.length > 0
      ? Math.min(...validTiers.map((t) => t.rent))
      : 0;
    ...
    const payload: any = {
      ...
      pricingTiers: validTiers.length > 0 ? validTiers : null,
      tenantTypes,
      availableFrom,
      maintenance,
      customMaintenance,
          ...
          <form onSubmit={handleNextStep}>
            ...
            {currentStep === 2 && (
              ...
              >
                ...
                {/* Multiple Pricing / Room Sharing Tiers Section */}
                <div className="bg-slate-50/90 border border-slate-200 rounded-2xl p-3.5 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <label className="block text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <span>Multiple Pricing Options / Sharing Tiers (Optional)</span>
                      </label>
                      <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                        Add variable rates for PG beds, hostels, or room categories (e.g., Single Sharing ₹20,000, 2-Sharing ₹15,000)
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => addPricingTier()}
                      className="px-2.5 py-1 bg-[#FF6B00] hover:bg-[#E56000] text-white rounded-xl text-[11px] font-extrabold transition-all shadow-xs cursor-pointer shrink-0 flex items-center gap-1 uppercase tracking-wider"
                    >
                      <span>+ Add Option</span>
                    </button>
                  </div>

                  {/* Quick PG & Room Presets */}
                  <div className="flex flex-wrap gap-1.5 items-center pt-1 border-t border-slate-200/60">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Quick Presets:</span>
                    {[
                      { name: "Single Sharing Room", rent: "20000" },
                      { name: "2-Sharing Bed", rent: "15000" },
                      { name: "3-Sharing Bed", rent: "12000" },
                      { name: "Private Suite", rent: "25000" },
                    ].map((preset) => (
                      <button
                        key={preset.name}
                        type="button"
                        onClick={() => addPricingTier(preset.name, preset.rent)}
                        className="px-2 py-0.5 rounded-lg bg-white hover:bg-[#FF6B00]/10 hover:text-[#FF6B00] border border-slate-200 text-[10px] font-extrabold text-slate-600 transition-colors shadow-2xs cursor-pointer"
                      >
                        + {preset.name}
                      </button>
                    ))}
                  ...
                  {/* Dynamic Tier Inputs List */}
                  {pricingTiers.length > 0 && (
                    <div className="space-y-2 pt-1">
                      {pricingTiers.map((tier, idx) => (
                        <div key={idx} className="p-2.5 bg-white border border-slate-200 rounded-xl grid grid-cols-12 gap-2 items-center shadow-2xs">
                          <div className="col-span-5">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider mb-0.5">
                              Price For / Option Name *
                            </label>
                            <input
                              type="text"
                              required
                              ...
                              placeholder="e.g. 2-Sharing Bed"
                              className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#FF6B00]"
                            />
                          </div>
                          <div className="col-span-3">
                            ...
                            <input
                              ...
                              required
                              value={tier.rent}
                              onChange={(e) => updatePricingTier(idx, "rent", e.target.value)}
                              placeholder="15000"
                              className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#FF6B00]"
                            />
                          </div>
                          <div className="col-span-3">
                            ...
                            <input
                              type="number"
                              value={tier.deposit || ""}
                              onChange={(e) => updatePricingTier(idx, "deposit", e.target.value)}
                              placeholder="30000"
                              className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#FF6B00]"
                            />
                          </div>
